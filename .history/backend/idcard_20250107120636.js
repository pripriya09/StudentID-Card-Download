...
import fs from 'fs';  // To handle file reading for storing as binary
...
// POST route to register a student and handle file upload
app.post('/api/students', upload.single('image'), async (req, res) => {
  const { name, fatherName, phoneNumber } = req.body;
  const { file } = req;  // The uploaded image will be in `req.file`
