...
const studentSchema = new mongoose.Schema({
  ...
  disease: { type: String, default: 'N/A' }, // Default to 'N/A' if not provided
  reference: {
    name: { type: String, required: true },
...
app.post('/api/students', async (req, res) => {
  ...
  try {
    ...
    for (const passenger of passengers) {
      ...
      if (!name || !fatherName || !phoneNumber || !image || !address || consent === undefined || !reference?.name || !reference?.mobileNumber) {
        return res.status(400).json({ error: 'All fields including reference details are required' });
      }
