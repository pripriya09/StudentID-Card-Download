...
const generateRegistrationNumber = async () => {
  lastRegistrationNumber++;  // Increment the last registration number
  const uniqueNumber = String(lastRegistrationNumber).padStart(4, '0');
  const registrationNumber = `${generateRandomPrefix()}${uniqueNumber}`;
...
app.post("/api/students", async (req, res) => {
  ...
  try {
    ...
    for (const passenger of passengers) {
      ...
      const registrationNumber = await generateRegistrationNumber();  // Generate unique registration number
...
app.delete("/api/students/:id", async (req, res) => {
  ...
  try {
    // Find the passenger by ID and delete it
    const deletedPassenger = await Student.findByIdAndDelete(passengerId);
