...
const handleDownloadPDF = (passengerId) => {
  ...
  // Create a hidden ID card element for PDF generation
  const idCard = document.createElement("div");
  idCard.style.position = "absolute";
  idCard.style.visibility = "hidden"; // Make it invisible, it won't be displayed on screen
  idCard.style.width = "210mm"; // Adjust to A4 width
  idCard.style.height = "297mm"; // Adjust to A4 height

  // Fill the card with the required content
  idCard.innerHTML = `
  ...
  // Append the hidden element to the DOM
  ...
  html2canvas(idCard).then((canvas) => {
    ...
    // Remove the hidden element from the DOM after the PDF is generated
