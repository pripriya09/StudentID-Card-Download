...
const PassengerTable = () => {
  ...
  const handleDownloadPDF = (passenger) => {
    if (!passenger) {
      alert("No passenger data available to generate ID card.");
      return;
    }
    ...
    html2canvas(idCard).then((canvas) => {
      ...
      pdf.addImage(imgData, "PNG", margin, margin, cardWidth, cardHeight);

      // Save the PDF with the passenger's name
