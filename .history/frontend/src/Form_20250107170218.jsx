  ...
  const handleDownloadPDF = () => {
    ...
    if (idCardElement) {
      html2canvas(idCardElement, { scale: 1 }).then((canvas) => {
        ...
        // Get the width and height of the element as it appears in the UI
        const idCardWidth = idCardElement.offsetWidth; // Width of the card in pixels
        const idCardHeight = idCardElement.offsetHeight; // Height of the card in pixels
  
        // Get the PDF's page size
        const pdfWidth = pdf.internal.pageSize.getWidth();
        const pdfHeight = (idCardHeight * pdfWidth) / idCardWidth;
  
        // Add the image to the PDF using the same width and height as the ID card in the UI
        pdf.addImage(imgData, "PNG", 0, 0, idCardWidth, idCardHeight);
  
        // Create a filename using the registration number
