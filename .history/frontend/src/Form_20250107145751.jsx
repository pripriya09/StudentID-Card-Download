...
const Form = () => {
  ...
  const handleDownloadPDF = () => {
    if (studentData) {
      ...
      // Capture the ID card as an image using html2canvas with scaling
      const captureElement = document.getElementById("id-card");
      html2canvas(captureElement, { 
        scale: 1.5,  // Adjust the scale for a better resolution while keeping the file size reasonable
        logging: false // Disable logging to improve performance
      }).then((canvas) => {
        // Convert the captured canvas to an image data URL
        const imageData = canvas.toDataURL("image/png");
  
        // Add the captured image (ID card) to the PDF
        PDFdocument.addImage(imageData, "PNG", 10, 10, 180, 250); // Adjust these dimensions as needed
  
        // Save the PDF with the ID card image
